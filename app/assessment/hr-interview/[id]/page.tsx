...
import HRInterviewClient from './HRInterviewClient';
...
export default async function HRInterviewPage({ params }: HRInterviewPageProps) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#0A0A18] via-[#0D0D20] to-[#0A0A18] flex items-center justify-center">
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-8 max-w-md">
          ...
          <p className="text-white/70">
            {interviewSession.error || 'Failed to load HR interview session'}
          </p>
        </div>
      </div>
    );
  ...
  const interviewConfig = {
    ...
    questionStyle: "conversational" as const, // HR interviews are conversational
  ...
  return (
    <RoleWrapper role={["candidate"]}>
      <Suspense fallback={<HRInterviewSkeleton />}>
        <HRInterviewClient 
...
function HRInterviewSkeleton() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0A0A18] via-[#0D0D20] to-[#0A0A18] flex items-center justify-center">
      <div className="text-white/70">Loading HR interview...</div>
    </div>
  );
}
